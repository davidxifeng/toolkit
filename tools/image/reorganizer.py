...
class ImageReorganizer:
    """A tool to reorganize horizontally concatenated images into 2D grid layouts."""
    
    def __init__(self):
        self.verbose = True
    
    def analyze_image(self, image_path, sub_width=None):
        """
    ...
    def find_best_layout(self, num_images, show_options=False):
        """
        ...
        if show_options and self.verbose:
    ...
    def create_grid_image(self, img, num_images, sub_width, sub_height, rows, cols, scale_factor=1.0):
        """
        ...
        for i in range(num_images):
            ...
            if self.verbose and ((i + 1) % 5 == 0 or i == num_images - 1):
                print(f"Processed {i + 1}/{num_images} images")
        
        return grid_img

    def reorganize(self, input_path, output_path=None, sub_width=None, scale_factor=1.0, show_layouts=False):
        """
        ...
        Args:
            ...
            show_layouts: Whether to show layout options
        ...
        if output_path is None:
            output_path = input_path.stem + "_grid" + input_path.suffix
        
        # Analyze the input image
        if self.verbose:
            print(f"Analyzing image: {input_path}")
        img, num_images, sub_width, sub_height = self.analyze_image(input_path, sub_width)
        
        # Calculate optimal layout
        rows, cols = self.find_best_layout(num_images, show_layouts)
        if self.verbose:
        ...
        grid_img = self.create_grid_image(img, num_images, sub_width, sub_height, rows, cols, scale_factor)
...
def main():
    """Command-line interface for the Image Reorganizer."""
    ...
    # Create reorganizer and process
    ...
    try:
        reorganizer.reorganize(
            ...
            show_layouts=args.show_layouts
        )
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
